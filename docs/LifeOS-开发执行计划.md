...
- ⚠️ **必须一并修复的集成缺口**：P2-3/P3-2 并行落地后，`health_page.dart`（饮食，入口=主页 life-hub「健康饮食」）与 `exercise_page.dart`（运动，路由 `/home/exercise`）是两个独立页，且**运动页当前是孤儿路由——全库无任何导航入口、用户点不进去**。P3-3 要把摄入/消耗整合进同一个"能量账本"健康视图（或至少在饮食页/主页补上运动页入口），确保运动页可达。（注：主页「运动」按钮开的是旧的宠物 `ExerciseDrawer`，非本页；两套运动流的最终统一在 P5-1。）
- 验收：[ ] 净值 = 摄入−消耗；[ ] 目标固定不被消耗抬高；[ ] **运动记录页有可达的 UI 入口（不再是孤儿路由）**
- 测试：净值计算测
